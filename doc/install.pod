...
You need to have WordNet (version 1.7.1 or 2.0), WordNet::QueryData
(version 1.38), and Text::OverlapFinder installed. Please note
that this version of WordNet-Similarity DOES NOT support WordNet 2.1
or WordNet::QueryData 1.39 as yet. 

WordNet is available at http://www.cogsci.princeton.edu/~wn/ and 
WordNet::QueryData is available from
http://search.cpan.org/dist/WordNet-QueryData/.  Text::OverlapFinder
is distributed with the Text-Similarity package and is available
from http://search.cpan.org/dist/Text-Similarity/.
...
previous versions of WordNet as well. The WordNet::QueryData Perl module is
...
QueryData 1.38 was used during the development. Also we observed that that
due to some major changes in QueryData from its previous versions, this
software does not work with the some earlier versions of QueryData. If you
have an earlier version of QueryData (1.29 or earlier) you may need to upgrade
QueryData.

=item 4.

Text::OverlapFinder: This module is used by the WordNet::Similarity::lesk
measure for finding runs of word overlaps in glosses.  The Text::OverlapFinder
module can be downloaded from http://search.cpan.org/dist/Text-Similarity/.
...
The following procedure will work on most Linux systems.
 ...
 wget http://search.cpan.org/CPAN/authors/id/J/JR/JRENNIE/WordNet-QueryData-1.38.tar.gz
 wget http://search.cpan.org/CPAN/authors/id/J/JA/JASONM/Text-Similarity-0.02.tar.gz
 wget http://search.cpan.org/CPAN/authors/id/T/TP/TPEDERSE/WordNet-Similarity-0.16.tar.gz

Then unpack each one:

 tar -zxvf WordNet-2.0.tar.gz
 tar -zxvf WordNet-QueryData-1.38.tar.gz
 tar -zxvf Text-Similarity-0.02.tar.gz
 tar -zxvf WordNet-Similarity-0.16.tar.gz
...
If you are using Linux, comment out the first line and uncomment the last
...
Installing QueryData and Similarity is much easier:
 ...
 cd /tmp/WordNet-Similarity-0.16
 perl Makefile.PL
 make
 make test
 su
