...
Berkeley DB: To use the WordNet::Similarity::vector measure, you need to
have Berkeley DB installed on your system. This can be freely downloaded
from http://www.sleepycat.com.  The vector measure is still experimental.
If you don't want to use the vector measure, you needed not worry about
installing Berkeley DB or the BerkeleyDB.pm module (see below).

=item 5.

BerkeleyDB.pm: You also need the Perl interface (BerkeleyDB.pm) be
installed on the system. This package was tested with version 0.20 of the
BerkeleyDB Perl module and version 4.1 of the Berkeley DB database. The
Perl module for BerkeleyDB can be obtained at CPAN:
